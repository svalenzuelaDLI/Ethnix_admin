...
export default function UploadZone({
 ...
 const [isUploading, setIsUploading] = useState(false);
  ...
  const startUpload = async(image: File[]) =>{
    ...
    console.log("UPLOADING HEREEE")
    const http = new HttpService();
    setIsUploading(true);

const dataupdate: IModel_NewCustomers.uploadImage={
  ResalesTaxCertificateImage: image[0],
  FederalTaxImge:image[0],
}
      ...
      if (setValue) {
        ...
        console.log("RESPONSE", response)
          ...
          // setFiles([]);
          // const respondedUrls = response?.map((r) => ({
          //   name: r.name,
          //   size: r.size,
          //   url: r.url,
          // }));
...
console.log("url", response.data.federalTaxImgeUrl)
           setValue(propertyname, response.data.federalTaxImgeUrl);
        }
        toast.success(
