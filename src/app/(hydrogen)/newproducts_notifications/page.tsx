...
import NewCustomersTable from '@/app/shared/newproducts_notifications/newproducts-list/table';
...
const pageHeader = {
  title: 'New Products Notifications',
  breadcrumb: [
    {
...
console.log("Session data --->",session)
  ...
  const spoolNotificationsRecords = async () => {    
    setLoading(true);
    const response = await http.service().get<IModel_NewCustomers.getNotifications>(`/items/v2/Notifications`,session?.user.access_token.user.token);
  ...
  return (
    <>
      ...
      <Link
          href={routes.newproducts_notifications.create}
